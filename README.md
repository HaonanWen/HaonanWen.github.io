...
Modern academic homepage template with a two-column landing page, YAML-driven content, selected publication cards, and an expanded publications page with optional demo preview support.
...
- `_data/news.yml`, `_data/honors.yml`, and `_data/publications.yml` as the main editable data sources
...
- YAML-driven content editing for news, honors, and publications
...
- Keeps news content lightweight and editable through YAML
...
2. Edit your personal info in `_config.yml` (name, role, institution, email).
3. Update content in `_data/*.yml` (social links, research interests, about, education, service, news, honors, publications).
...
5. Run the site locally:
   ...
   ```
6. Open http://localhost:4000 and deploy to GitHub Pages.
...
- Relative asset paths in data files are handled automatically for subpages under `pages/`.
